...
class DbtArtifacts:
    ...
    def get_cli_resource(self, **kwargs) -> DbtCliResource:
        ...
        If packaged_project_dir is set, resolves to that outside of the development context and
        uses project_dir when in a development context.
        """
        return DbtCliResource(
            project_dir=os.fspath(self._current_project_dir),
            **kwargs,
        )
    ...
    def _prepare_manifest(self, level: int = logging.DEBUG) -> Path:
        logger.log(level, f"Preparing dbt artifacts in {self.project_dir}.")
        return (
