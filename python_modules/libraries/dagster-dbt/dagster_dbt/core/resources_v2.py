...
# TODO: Need to figure out a better place for this to live so it isn't globally scoped.
...
class DbtCliEventMessage:
    ...
    def to_default_asset_events(
        ...
        if (
            ...
            lineage_metadata = (
                self._process_lineage_metadata(
                    manifest=manifest,
    ...
    def _process_lineage_metadata(
        ...
        # If the unique_id is a seed or source, then we don't need to process lineage.
        if unique_id.startswith("seed") or unique_id.startswith("source"):
            return {}

        # 1. Retrieve all of the node ancestors and their SQL files (if they exist)
        qualified_table_name = cast(str, event_node_info["node_relation"]["relation_name"]).replace(
            '"', ""
        )
        ...
        sqlglot_sources[qualified_table_name] = optimized_node_ast
        sqlglot_schema.add_table(cast(exp.Table, optimized_node_ast))

        # 2. Retrieve the column names from the current node.
        column_names = cast(exp.Query, optimized_node_ast).named_selects

        # 4. For each column, retrieve upstream columns dependencies from direct parents.
        for column_name in column_names:
            ...
            for sqlglot_lineage_node in lineage(
                ...
                sources=sqlglot_sources,  # type: ignore
            ).walk():
